function [data, data_opt,profits_opt] = PointForecast_optimization( system_data, wind_data, market_data, main_data)
data=[];
data_opt=[];
profits_opt=[];
...
main_data.prob=1;
%pw=point_wind/e_max;
wind_data.scenarios=wind_data.point_forecast/system_data.e_max;
...
%                
    ...
    'Hessian','on','HessFcn',@(x,lambda)ExpectedValue_hessian(x,lambda,n_var));%notify iter
        ...
        [x,fval,exitflag,output,lambda,grad,hessian] = fmincon(@(x)PointForecast_objectiveFunction(x,d,n_var,system_data, wind_data, market_data, main_data),x0,Ad,bd,Aeq,beq,lb,ub,[],options);
...
%         dados(:,2*s+1+1)=power_to_grid(:,1);
        ...
        filename=sprintf('%s%s%d%s%d',fullpath,'PointForecast_from_', main_data.sd, '_to_', d);
